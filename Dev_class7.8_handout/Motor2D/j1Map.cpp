...
void j1Map::UnvisitedPath(int x, int y)
{
	path.Clear();
	iPoint goal = WorldToMap(x, y);
	...
	// TODO 2: Follow the breadcrumps to goal back to the origin
	// add each step into "path" dyn array (it will then draw automatically)
	if (breadcrumbs.start != nullptr && breadcrumbs.find(goal) != -1) {

		p2List_item<iPoint> *item = breadcrumbs.At(breadcrumbs.find(goal));
		iPoint current/* = goal*/;

		while (item != breadcrumbs.start) {

			current = item->data;
			path.PushBack(current);
			item = item->prev;
			//current = breadcrumbs.At(breadcrumbs.find(current))->data;
...
void j1Map::PropagateDijkstra()
{
	...
	if (frontier.Pop(current))
	{
		...
		for (uint i = 0; i < 4; ++i)
		{
			...
			if (cost != cost_so_far[current.x][current.y] - 1 &&
				(cost_so_far[neighbors[i].x][neighbors[i].y] == 0 || cost_so_far[neighbors[i].x][neighbors[i].y] > cost)) {
...
void j1Map::PropagateAStar(iPoint goal)
{
	...
	// on each cell (is already reset to 0 automatically)
	iPoint current;
	if (frontier.Pop(current) && visited.find(goal) == -1)
	{
		...
		iPoint next = neighbors[0];
		int cost = cost_so_far[current.x][current.y] + MovementCost(neighbors[0].x, neighbors[0].y) + ManhattanDistance(neighbors[0], goal)*10;

		for (int j = 0; j < 4; ++j) {

			int check_cost = cost_so_far[current.x][current.y] + MovementCost(neighbors[j].x, neighbors[j].y) + ManhattanDistance(neighbors[j], goal)*10;
		...
		cost_so_far[next.x][next.y] = cost;
		visited.add(next);
		breadcrumbs.add(current);
	...
	else if(visited.find(goal) != -1)
		App->map->Path(goal.x - App->render->camera.x, goal.y - App->render->camera.y);

}

int j1Map::ManhattanDistance(iPoint start, iPoint end) {
